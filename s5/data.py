...
@partial(jax.jit, static_argnums=(1, 2, 3))
def create_constructed_reg_data_new(rng, i_size, c_size, size_distract, input_range, w_scale):
  """Create a linear regression data set: X*w where x ~ U(-1, 1), w ~ N(0,1)."""
  ...
  x_y_combined = x * y_data[:, np.newaxis] # x*y
  # x_next = jnp.vstack((x[1:], x_querry))
  x_next = jnp.vstack((x_y_combined[1:], x_querry))
  seq = jnp.concatenate((x_y_combined, x_next), axis=1) #[x_1*y_1,x_2*y_2]
