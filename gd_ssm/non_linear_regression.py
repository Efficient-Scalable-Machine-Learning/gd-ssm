...
from gd_ssm.train_helpers import linear_warmup, cosine_annealing, constant_lr, train_epoch,\
        validate
from gd_ssm.analysis import analyse,scan_lrs,nfeature_analyse
...
def gd_init(args,state,lr):
    if args.n_layers ==1:
        return state
    else:
            #gd_model_cls,gd_state = model_init(args,rng,gd_params=False,gd_lr=lr)
                ## initialising LSA weights
            state.params['layers_0']['seq']['w_q'] = jnp.outer(jnp.array([1.0,0,0]), jnp.array([0,1.0,0]))
            state.params['layers_1']['seq']['w_q'] = jnp.outer(jnp.array([1.0,0,0]), jnp.array([1.0,0,0]))
            state.params['layers_2']['seq']['w_q'] = jnp.outer(jnp.array([1.0,0,0]), jnp.array([0,1.0,0]))
            ## skipweight
            state.params['layers_2']['seq']['D'] = jnp.array([0, 0, 1]) *1.0
            ## Recurrent weights
            state.params['layers_0']['seq']['Lambda_re'] = jnp.ones((args.ssm_size_base,args.ssm_size_base))
            state.params['layers_1']['seq']['Lambda_re'] = jnp.ones((args.ssm_size_base,args.ssm_size_base))
            state.params['layers_2']['seq']['Lambda_re'] = jnp.ones((args.ssm_size_base,args.ssm_size_base))          
            ## Learning rate parameters
            state.params['layers_0']['seq']['C'] = jnp.eye(args.ssm_size_base)*(-lr/args.ssm_size_base)
            state.params['layers_1']['seq']['C'] = jnp.eye(args.ssm_size_base)*(-lr/args.ssm_size_base)
            state.params['layers_2']['seq']['C'] = jnp.eye(args.ssm_size_base)*(-lr/args.ssm_size_base)
    return state
def train(args):
    """
    ...
    if args.n_layers ==1:
        if args.analyse or args.ood_analyse:
            ...
            gd_model_cls,gd_state = model_init(args,init_rng,gd_params=True,gd_lr=gd_lr) #FIXME : 
    ...
    for epoch in range(args.epochs):
        ...
        val_loss = validate(state,
                            ...
                            args.batchnorm,
                            args.dataset)
#        if (epoch<1000) & (args.n_layers !=1): Train anyways for non-linear
        if args.analyse:
                gd_state, gd_train_loss, step = train_epoch(gd_state,
                                                ...
                                                args.dataset,
                                                lr_params)
                gd_state = gd_init(args,gd_state,gd_lr)
        ...
        if args.ood_analyse:
            import numpy as np
            ### OOD Tasks - Analyse alignement between GD and trained Transformer on OOD settings
 ...
 #save model checkpoint
    ...
    #checkpoints.save_checkpoint(ckpt_dir=os.path.join(os.path.abspath('checkpoints_gd'),'checkpoints'), target=state, step=0,overwrite=True)
    

### Analysis
    if args.analyse:
        import numpy as np
        from gd_ssm.analysis import display_learning
        cosine_low = 0.0
